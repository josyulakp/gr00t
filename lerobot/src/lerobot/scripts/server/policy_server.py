...
MODEL_PATH = "/home/josyula/projects/gr00t_checkpoint/checkpoint-12000" # "nvidia/GR00T-N1.5-3B"
...
class PolicyServer(services_pb2_grpc.AsyncInferenceServicer):
    ...
    def _get_action_chunk(self, observation: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        chunk_combined = np.concatenate([chunk["action.single_arm"], chunk["action.gripper"].reshape(-1,1)], axis=1)
        return chunk_combined[:, : self.actions_per_chunk]
    ...
    def _predict_action_chunk(self, observation_t: TimedObservation) -> list[TimedAction]:
        ...
        """2. Get action chunk"""
        start_time = time.perf_counter()
        action_tensor = self._get_action_chunk(observation_t.get_observation())
        inference_time = time.perf_counter() - start_time

        """3. Post-inference processing"""
        start_time = time.perf_counter()
        # Move to CPU before serializing
        # action_tensor = action_tensore.cpu().squeeze(0)

        action_chunk = self._time_action_chunk(
            observation_t.get_timestamp(), action_tensor.tolist(), observation_t.get_timestep()
        )
