...
def execute_actions_loop(robot: RobotInterface, gripper:GripperInterface, fps=FPS):
    ...
    Kx_scale = Kxd_scale = 0.001
    Kx = np.array([75.0, 75.0, 75.0, 15.0, 15.0, 15.0]) * Kx_scale/20.0
    Kxd = np.array([37.0, 37.0, 37.0, 2.0, 2.0, 2.0]) * Kxd_scale/20.0

    # Start controller once (non-blocking)
    # robot.start_joint_impedance(blocking=False)
    robot.start_cartesian_impedance(Kx.tolist(),
                            Kxd.tolist(),
                            blocking=False)
    ...
    while True:
        try:
            ...
            pos = q_cmd[:3]
            # First convert GR00T quaternion [qw,qx,qy,qz] → [x,y,z,w]

            quat = q_cmd[3:]
            # quat_xyzw = [quat[1], quat[2], quat[3], quat[0]]

            # Initialize memory for smoothing
            if playhead == 0:
                prev_pos, prev_quat = pos, quat
            ...
            slerp = Slerp([0, 1], R.from_quat([prev_quat, quat]))
            rot = slerp([alpha])[0].as_quat()

            prev_pos, prev_quat = pos, rot  # update memory
