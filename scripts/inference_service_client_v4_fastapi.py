...
def gr00t_fastapi_client_call(obs: dict, host: str, port: int):
    """
    Send observation to GR00T inference server via FastAPI HTTP POST.
    Obs is pickled + base64 encoded.
    """
    url = f"http://{host}:{port}/infer"
    obs_b64 = base64.b64encode(pickle.dumps(obs)).decode("utf-8")
    
    try:
        # resp = requests.post(url, data={"observation_b64": obs_b64})
        resp = session.post(url, data={"observation_b64": obs_b64})
        resp.raise_for_status()
        data = resp.json()

        if not data.get("success", False):
            raise RuntimeError(f"Server error: {data.get('error', 'Unknown error')}")

        actions = pickle.loads(base64.b64decode(data["result_b64"]))
        return actions

    except Exception as e:
        print(f"[Client] Error calling FastAPI server: {e}")
        return None
...
def fetch_actions_loop(args):
    ...
    while True:
        try:
            ...
            actions = gr00t_fastapi_client_call(obs, args.host, args.port)
