...
def main(args: ArgsConfig):
    if args.server:
        ...
        data_config = DATA_args.clientCONFIG_MAP[args.data_config]
    ...
    # Here is mainly a testing code
        ...
        last_closed = 0
        prev_actions = None
        print("waiting for gripper opening")
        gripper.open_async(1.0)
        
        time.sleep(2.0)
        print("wait time done ")
        while True:
            ...
            try:
                ...
                # print(f"wrist stats in each channel {np.mean(wrist_img, axis=(0,1))}")
                # print(f"left stats in each channel {np.mean(left_img, axis=(0,1))}")
                # print(f"right stats in each channel {np.mean(top_img, axis=(0,1))}")
            ...
            print("http server ", args.http_server)
            if args.http_server:
                print("http server call")
            ...
            if prev_actions is not None:
                smoothed_actions = get_lipo_actions(actions["action.single_arm"], prev_actions)
            # print("LP", smoothed_actions)

            # if len(smoothed_actions)>0:
            #     joint_waypoints = []
            #     for i in range(smoothed_actions.shape[0]):
            #         if(i% 4 == 0):
            #             # print(f"Executing action {i}")
            #             action = smoothed_actions[i][:7]
            #             robot.move(franky.JointMotion(action.tolist()))
            #             # print("Gripper action:", actions[i][-1])
            # else:         
            # import ipdb; ipdb.set_trace()
            last = actions["action.single_arm"][-1].tolist() 
            delta = actions["action.single_arm"][-1] - actions["action.single_arm"][0]
            mean = actions["action.single_arm"].mean(axis=0)
            m_jp = franky.JointMotion(mean.tolist())

            robot.move(m_jp)

            print("mean ", actions["action.gripper"].mean())
            if last_closed <= 10:
                if actions["action.gripper"][-4:].mean() <= 0.035:
                    print(last_closed)
                ...
                gripper.open_async(1.0)
                last_closed = 0
            prev_actions = actions["action.single_arm"][0]
            prev_gripper = actions["action.gripper"]
